...
exports = module.exports = {
  upload: function(req, res) {
    ...
    if (req.files && req.files.file) {
      ...
      var headers = Types.S3File.prototype.generateHeaders.call(
        { s3config: s3Config, options: {} },
        null,
        file
      );
      ...
      s3Client.putFile(file.path, path + file.name, headers, function(
        err,
        s3Response
      ) {
        var sendResult = function() {
          ...
          if (s3Response) {
            if (s3Response.statusCode !== 200) {
              return res.send({
                error: {
                  message: "Amazon returned Http Code: " + s3Response.statusCode
                }
              });
            } else {
              return res.send({
